...
export default async function CookiePolicyPage() {
  ...
  try {
    ...
    if (settings?.cookiePolicy?.trim() && !isUrl(settings.cookiePolicy)) {
      content = settings.cookiePolicy.trim();
    }
  } catch (e) {
    console.error("Cookie policy settings fetch error:", e);
  }

  return (
    <main className="min-h-screen bg-white">
      <PolicyPageHero title="Çerez Politikası" variant="cookie" />
            ...
            <CardContent className="p-6 sm:p-8 md:p-10">
              {content ? (
                ...
                <p className="text-center text-text-muted">
                  Çerez politikası metni henüz eklenmemiş veya yüklenemedi.
                </p>
              )}
            </CardContent>
          </Card>
